...
const Cart: React.FC<Props> = ({ onClose, title }): React.ReactElement => {
  ...
  return (
    <div className="font-poppins h-full flex flex-col">
      ...
      />
      <div className="flex-1 overflow-y-auto  sm:my-4">
        <div className="flex flex-col  justify-center mx-6 my-4 sm:mt-0 gap-4 text-gray lg:max-h-[830px] ">
          ...
          {userId && basketProducts.length ? (
            <div>
              <div className={' bg-custom-turquoise flex items-center justify-center px-4 py-2'}>
                <p className={`${styles.body1} lg:text-[18px] text-center`}>
                  The items in your cart are not reserved. Check out now to make them yours.
                </p>
              </div>

              <div className="h-auto mt-6 lg:flex lg:gap-2">
