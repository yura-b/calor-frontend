        ...
        color.name !== null ? (
          <Tooltip key={color.name} title={color.name} placement="top" arrow>
            <button
              ...
              className="min-h-[55px] min-w-[55px] rounded-full shadow focus:drop-shadow-2md focus:outline-none ring-2 focus:ring-4 ring-grayLight focus:ring-mint"
              onClick={() => handleColorClick({ img: color.img, name: color.name })}
            >
            {
              color.texture ? <img src={color.texture} alt={color.name} height={55} width={55} /> : null
            }
            </button>
          </Tooltip>
        ) : null
