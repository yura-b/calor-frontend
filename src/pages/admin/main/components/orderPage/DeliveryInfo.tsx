...
const DeliveryInfo:FC<{_id: string}> = ({_id}) => {
    ...
    return (
        <div className={'flex flex-col w-1/2 mt-4'}>
            ...
            {openDeliveryInfo ?  (
                <div>
                    <CustomInput value={courier} onChange={changeHandler(setCourier)}>Courier</CustomInput>
                    <CustomInput value={trackingNumber} onChange={changeHandler(setTrackingNumber)}>trackingNumber</CustomInput>
                    <CustomInput value={approxDeliveryDate} onChange={changeHandler(setApproxDeliveryDate)}>approxDeliveryDate</CustomInput>
                </div>
            ) : (
                <p>
                    Delivery info
            ...
            {openDeliveryInfo? <CustomButton title={'Start production'} handler={uploadDeliveryInfo}/> :  <CustomButton title={'+ add delivery information'} handler={() => {
