...
const OrderGridPage = () => {
    ...
    filteredOrder = filteredOrder.filter((order) => {
        ...
        if (chosenSection === chosenOrders.history) return order.status === OrderStatus.Shipped || order.status === OrderStatus.Refunded
            ...
            <div className={'flex flex-row mb-6'}>
                {sections.map((section) => {
                    if (section === chosenSection) {
                        return (
                            <div key={section} className={defaultStyles + chosen}
