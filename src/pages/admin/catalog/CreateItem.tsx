...
const CreateItem = () => {
    ...
    const sizeHandler = (setState: React.Dispatch<React.SetStateAction<{
        index: number,
        size: number
    }[]>>, index: number) => {
        return (e: SelectChangeEvent) => {
    ...
    return (
        <AdminLayout>
            <UserPageHeader buttonAvailable={false} url={'/admin/catalog'} upperText={'Back to catalog'}
                            bottomText={'New Item'}/>
            <div className={'m-24'}>
                ...
                <div className={'flex flex-row gap-12'}>
                    <div className={'flex flex-col gap-5 w-1/2'}>
                        ...
                        {photos?.map(photo => {
                            return <div key={photo.name} className={'flex flex-row justify-between align-baseline'}>
                    ...
                    <div className={'flex flex-col gap-5 w-1/2'}>
                        ...
                        <CustomSelect value={selectedCategory} array={categories.map(el => el.categoryTitle)}
                                      defaultValue={''}
                                      handleFunc={changeHandler(setSelectedCategory)}/>

                        <CustomSelect value={valueOfSubCategory} array={subcategories} defaultValue={''}
                                      handleFunc={changeHandler(setSelectedSubCategory)}/>

                        <CustomInput description={'Product name'} value={productName}
                                     onChange={changeHandler(setProductName)} border={border}/>

                        <p>Description</p>
                        <CustomTextField defaultValue={description} setValue={textAreaHandler(setDescription)}
                                         disabledField={false}/>

                        <CustomInput type={InputType.number} description={'Price'} value={price}
                                     onChange={changeHandler(setPrice)} border={border}/>

                        <CustomInput type={InputType.number} description={'Amount'} value={amount}
                                     onChange={changeHandler(setAmount)} border={border}/>

                        <div className={'flex flex-row flex-wrap gap-6'}>
                            {Array.from({length: sizes.length}).map((_, index) => {
                                const value = sizes.find(size => size.index === index+1)?.size.toString()
                                return <div key={Math.random()} className={'w-1/5'}> <CustomInput type={InputType.number} value={value}
                                                    onChange={sizeHandler(setSizes, index + 1)} border={border}/>
                                </div>
                            })}
