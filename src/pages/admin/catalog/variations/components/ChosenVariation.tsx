...
const ChosenVariation:FC<IProps> = ({handler,products, saveHandler}) => {
    return (
        <div className={'fixed w-5/6 right-0 bottom-0 bg-custom-grey border-r border-white border-[2px] p-5 z-10 overflow-x-auto'}>
                ...
                {products.map(({ photos, title, _id }) => {
                    return (
                        <div key={_id} className={'flex flex-col gap-2 w-1/5 items-center mt-4'} onClick={() => handler(_id)}>
                            <img src={photos[0]} alt={'photo'} className={'aspect-[2/1] h-[115px] cursor-pointer object-contain hover:fill-red'} />
